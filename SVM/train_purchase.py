...
for loop in range(30):
    index = [5,9]
    pre_data= []
    data = [0 for i in range(len(origin_purchase))]
    for k in range(len(index)):
        pre_purchase = [0 for l in range(len(origin_purchase))]
        for i in range(len(origin_purchase)):
            if i < index[k]:continue
            for l in range(index[k]):
                pre_purchase[i] += origin_purchase[i - 1 - l]

        mu = np.mean(pre_purchase)
        st = np.std(pre_purchase)
        for i in range(len(origin_purchase)):
            pre_purchase[i] = (pre_purchase[i] - mu) /st
        pre_data.append(pre_purchase)

    mu = np.mean(origin_purchase)
    st = np.std(origin_purchase)
    for i in range(len(origin_purchase)):
        data[i] = (origin_purchase[i] - mu)/st
    train_x = []
    test_x = []
    train_y = []
    test_y = []
    ...
    pre_x.append(data[d - 42])
    pre_x.append(data[d - 35])  
    ...
    pre_y = pre_y*st + mu
    print pre_y
    origin_purchase.append(pre_y[0])

f = file('result_purchase.pkl','wb')
pickle.dump(origin_purchase,f)
f.close()
