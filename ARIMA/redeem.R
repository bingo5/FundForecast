...
acf = acf(data_diff1[2:length(x)],lag.max = 100,plot = FALSE)
plot(acf)

pacf = pacf(Data,lag.max = 100,plot = FALSE)
#plot(pacf)
library(forecast)
data.fit = arima(Data,order = c(1,1,2),seasonal = list(order = c(1,0,1),period = 7))
forecast = forecast.Arima(data.fit,h = 30,level = c(99.5))
plot.forecast(forecast)
write.table(forecast$mean,file = "redeem_data.csv",row.names = F,quote = F)
